...
<div class="d-flex flex-wrap justify-content-between align-items-center mb-4 ds-animate">
    <div>
        <h1 class="fw-bold mb-1" style="font-size: 1.75rem;">
            <i class="bi bi-bookmarks text-primary me-2"></i>Document Categories
        </h1>
        <p class="text-muted mb-0">Manage categories to organize your documents</p>
    </div>
    <div class="d-flex gap-2">
        <a href="{{ route('admin.dashboard') }}" class="btn btn-outline-secondary">
            <i class="bi bi-arrow-left"></i> Dashboard
        </a>
        <a href="{{ route('admin.categories.create') }}" class="btn btn-primary" id="addCategoryBtn">
                        ...
                        @foreach($categories as $category)
                            <tr>
                                ...
                                <td>
                                    @if($category->user)
                                        <div class="d-flex align-items-center gap-2">
                                            <div class="ds-user-avatar" style="width:24px;height:24px;font-size:0.6rem;background:linear-gradient(135deg,{{ $category->user->isAdmin() ? '#7c3aed,#a855f7' : '#0d9488,#0d6efd' }});">
                                                {{ strtoupper(substr($category->user->name, 0, 1)) }}
                                            </div>
                                            <span class="small">{{ $category->user->name }}</span>
                                        </div>
                                    @else
                                        <span class="badge bg-secondary">Global (System)</span>
                                    @endif
                                </td>
                                <td>
